...
df = pd.DataFrame(columns=('OneVsRestClassifier', 'BaggingClassifier', 'MultinomialNB', 'mean', 'std','Precision','Recall','F-measure','True Postives','True Negatives','False Positives'))
i = 0
for w1,w2,w3 in zip([1,3,3],[2,1,1],[1,2,1]):
    ...
    for a,b in zip(y_test,y_pred):
        if(a==0 and b==0):
            ...
            TN = TN+1
    scores = model_selection.cross_val_score(estimator=eclf,X=X,y=y,cv=5,scoring='accuracy',n_jobs=1)
    scores1 = metrics.precision_recall_fscore_support(y_test,y_pred,average=None)
    df.loc[i] = [w1, w2, w3, scores.mean(), scores.std(),scores1[0].mean(),scores1[1].mean(),scores1[2].mean(),TP,TN,FP]
    i += 1
print(df)
"""i = 0
for w1 in range(1,4):
